...
function eat()
{
    ...
    if (adb shell getprop ro.aosgp.version | grep -q "$LINEAGE_BUILD");
    then
...
function installboot()
{
    ...
    if (adb shell getprop ro.aosgp.version | grep -q "$CM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.aosgp.version | grep -q "$CM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
